...
typedef struct {
	...
	uint32_t tiempo;
} task_param_t; // Parametrizo la tarea
...
static task_param_t task_param1 = {
		...
		.led_gpio_pin = LED1_Pin,
		.tiempo = 1000
};

static task_param_t task_param2 = {
		.led_gpio_port = LED2_GPIO_Port,
		.led_gpio_pin = LED2_Pin,
		.tiempo = 500
};
...
static void task(void *a)
{
	...
	while (1) {
		HAL_GPIO_TogglePin(task_param->led_gpio_port, task_param->led_gpio_pin);
...
int main(void)
{
  ...
  xTaskCreate(task, (const char *)"task_1",
		  configMINIMAL_STACK_SIZE*2, &task_param1, osPriorityNormal, 0);

  xTaskCreate(task, (const char *)"task_2",
		  configMINIMAL_STACK_SIZE*2, &task_param2, osPriorityNormal, 0);
